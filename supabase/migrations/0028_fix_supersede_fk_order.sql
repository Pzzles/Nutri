-- 0028_fix_supersede_fk_order.sql
--
-- Fixes a FK-violation bug in fn_start_goal_phase_v2 when transition=supersede.
--
-- goal_phases.superseded_by has a self-referential FK:
--   superseded_by UUID REFERENCES public.goal_phases(id)
--
-- Migration 0022 inserted the new phase AFTER updating the old phase's
-- superseded_by field. Because the new phase didn't exist yet, the UPDATE
-- violated the FK constraint and the RPC raised a DB exception, causing the
-- edge function to return INTERNAL_ERROR.
--
-- Fix: in the supersede branch, only set status/ended_at immediately.
-- Back-fill superseded_by after the new phase row exists (Step 2).
...
BEGIN
  ...
  -- NOTE: for supersede we do NOT set superseded_by here because the new phase
  -- row (v_new_phase_id) doesn't exist yet and the self-referential FK would
  -- reject the UPDATE.  superseded_by is back-filled in Step 2b below.
  ...
  -- ── Step 2a: insert the new goal phase (snapshot_id NULL for now) ──────────
  ...
  INSERT INTO public.goal_phases (
    ...
    NULL, p_manual_maintenance_kcal
  );

  -- ── Step 2b: back-fill superseded_by now that the new phase row exists ──────
  IF p_transition = 'supersede' AND v_existing_id IS NOT NULL THEN
    UPDATE public.goal_phases
    SET superseded_by = v_new_phase_id,
        updated_at    = now()
    WHERE id = v_existing_id;
  END IF;

  -- ── Step 3: insert the snapshot (now goal_phase_id can reference step 2a) ──
