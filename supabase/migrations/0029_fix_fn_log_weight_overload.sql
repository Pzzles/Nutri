-- 0029_fix_fn_log_weight_overload.sql
--
...
--
-- PostgREST returns PGRST203 (ambiguous function) when callers use
-- named-parameter RPC because it cannot choose between the two overloads.
--
-- Fix: drop the original 5-parameter signature. The 6-parameter version (with
-- p_source TEXT DEFAULT 'manual') is fully backwards-compatible.

DROP FUNCTION IF EXISTS public.fn_log_weight(UUID, NUMERIC, TIMESTAMPTZ, DATE, TEXT);
