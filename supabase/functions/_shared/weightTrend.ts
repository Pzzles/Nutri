// Pure, side-effect-free weight trend calculations for Phase 6.
// No I/O, no Supabase calls. All functions accept plain data and return plain data.
//
// Algorithm flow:
//   raw measurements
//     → sortChronologically
//     → applyEWMA          (smoothed trend line)
//     → detectOutliers     (flag statistically extreme points)
//     → linearRegression   (weekly rate over TREND_REGRESSION_WINDOW_DAYS)
//     → assessConfidence   (low / medium / high)
//     → buildTrendResult   (structured response)
...
export function detectOutliers(points: EWMAPoint[]): EWMAPoint[] {
  ...
  return points.map((p, i) => {
    ...
    // Statistical outlier: residual > OUTLIER_RESIDUAL_SIGMA standard deviations
    const isStatistical = sd > 0 && residual > OUTLIER_RESIDUAL_SIGMA * sd;

    // Biological plausibility: single-day jump > 10% of body weight
    ...
    if (i > 0) {
      ...
      const dailyFraction =
        Math.abs(p.raw_weight_kg - prev) / prev / daysDiff;
...
export function calculateWeightTrend(
  ...
  // Max gap between consecutive measurements
  let maxGapDays = 0;
  ...
  // Regression (only if minimum data criteria met)
  ...
  // Warnings
  if (sorted.length < TREND_MIN_MEASUREMENTS_FOR_RATE) {
    warnings.push("insufficient_measurements");
  }
  if (coverageDays < TREND_MIN_COVERAGE_DAYS_FOR_RATE) {
    warnings.push("insufficient_coverage");
  }
  if (daysSinceLatest > 14) {
    warnings.push("stale_data");
  }
  if (maxGapDays > 21) {
    warnings.push("large_gap");
  }
